---
title: 'EDS 223: assignment 4 rubric'
author: "Ruth Oliver"
date: "2022-11-16"
output:
    html_document:
...
wc_rgns <- st_read(dsn = 'data', layer = 'wc_regions_clean')
...
sstAvg <- sstAvg - 273.15

depth <- crop(depth, sstAvg)
depth_resample <- terra::resample(depth, y = sstAvg, method = "near")

c(depth_resample, sstAvg)
```
...
```{r include=TRUE}
rcl1 <- matrix(c(-Inf, 11, NA, 
                 11, 30, 1, 
                 30, Inf, NA), ncol = 3, byrow = TRUE)

sstPref <- classify(sstAvg, rcl = rcl1)
plot(sstPref, col = "red", main = "areas with suitable SST")

rcl2 <- matrix(c(-Inf, -70, NA, 
                 -70, 0, 1, 
                 0, Inf, NA), ncol = 3, byrow = TRUE)
depthBin <- classify(depth_resample, rcl = rcl2)
plot(depthBin, col = "red", main = "areas with suitable depth")

cells = lapp(c(sstPref, depthBin), fun=function(x, y){return(x*y)})
plot(cells, col = "red", main = "suitable aquaculture areas")
```
...
```{r include=TRUE}
cellsEEZ <- mask(cells, wc_rgns)
plot(cellsEEZ, col = "red")

wcZones <- rasterize(wc_rgns, cellsEEZ, field = "rgn_id")
plot(wcZones)

cellsArea <- cellSize(cellsEEZ, unit = "m")
plot(cellsArea)
...
west_coast_aq$region_area <- as.numeric(st_area(west_coast_aq))

west_coast_aq <- west_coast_aq %>%
  mutate(percent_suitable = (suit_aq_area/region_area)*100)
...
```{r include=TRUE}
tm_shape(west_coast_aq) +
  tm_polygons(col = "suit_aq_area")
